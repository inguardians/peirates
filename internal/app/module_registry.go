...
func newModuleRegistry(session *Session) *modules.Registry {
	...
	registry.Register(func() modules.Result {
		if err := launchHostProcCorePatternBreakout(); err != nil {
			fmt.Fprintf(os.Stderr, "[hostproc-core-pattern-breakout] %v\n", err)
		}
		return modules.Continue
	...
	registry.Register(func() modules.Result {
		...
		if err := launchNodesProxyExec(session.Connection, accounts); err != nil {
			fmt.Fprintf(os.Stderr, "[nodes-proxy-exec] %v\n", err)
		}
		return modules.Continue
	}, "nodes-proxy-exec")
	registry.Register(func() modules.Result {
