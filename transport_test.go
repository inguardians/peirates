...
func TestKubectlAuthCanI(t *testing.T) {
	previous := UseAuthCanI
	t.Cleanup(func() { UseAuthCanI = previous })
	UseAuthCanI = false
	if !kubectlAuthCanI(ServerInfo{}, "get", "pods") {
		t.Fatal("disabled auth check should allow request")
	}
	UseAuthCanI = true
	if !kubectlAuthCanI(ServerInfo{ClientCertData: "cert"}, "get", "pods") {
		t.Fatal("certificate auth is currently allowed without an API check")
	}
	if kubectlAuthCanI(ServerInfo{APIServer: "https://127.0.0.1:1"}, "get", "pods") {
